...
export default async function Page({ params }) {
	...
	const totalUserConversation = await prisma.Conversation.findMany({
		where: {
			userId: id,
		},
	})

	const conversationCountViaModel = {}
	totalUserConversation.forEach(conversation => {
		const modelName = conversation.modelName.toLowerCase()
		conversationCountViaModel[modelName] = (conversationCountViaModel[modelName] || 0) + 1
	})

	allModels.forEach(model => {
		model.totalConversation = conversationCountViaModel[model.link] || 0
	})
			...
			<div>
				<div className="flex h-screen antialiased text-gray-800">
					<div className="flex flex-row h-full w-full overflow-x-hidden">
						<div className="hidden md:flex flex-col py-8 px-4 w-64 bg-white flex-shrink-0">
